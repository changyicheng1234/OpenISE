        ...
        try:
            ...
            if isinstance(result, dict):
                formatted_result.update({
                    ...
                    '总热量': result.get('总热量', '0 千卡'),                    '蛋白质': result.get('蛋白质', '0 克'),
                    '脂肪': result.get('脂肪', '0 克'),
                    '碳水化合物': result.get('碳水化合物', '0 克'),
                    '整体评分': result.get('整体评分', 0.0)
                })
    ...
    if request.method == 'POST':
        try:
            ...
            result = generate_economical_diet(
                agent,
                data.get('budget', '40'),
                data.get('location', '北京'),
                data.get('health_goal', ''),
                data.get('dietary_preferences', data.get('preferences', ''))
            )
...
@app.route('/weight_loss', methods=['GET', 'POST'])
def weight_loss():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # 创建专属agent
            agent = create_diet_agent(WEIGHT_DIET_ROLE)
            ...
            result = generate_weight_loss_plan(
                ...
                days=int(data.get('days', 7)),  # 默认7天计划
                # 从用户档案获取补充参数
            ...
            formatted_result = {
                '计划概览': {
                    '当前体重': result['计划概览'].get('当前体重', 'N/A'),
                    '目标体重': result['计划概览'].get('目标体重', 'N/A'),
                    '计划周期': result['计划概览'].get('计划周期', '7天'),
                    '每日热量': f"{result['计划概览'].get('每日热量', 0)}大卡",
                    '营养分配': result['计划概览'].get('总营养', {
                        'calories': 0,
                        'protein': 0,
                        'fat': 0,
                        'carbs': 0
                ...
                '每日食谱': {
                    '早餐': {
                        '菜单': result['每日食谱'].get('breakfast', {}).get('菜单', []),
                        '营养': result['每日食谱'].get('breakfast', {}).get('营养分析', {})
                    },
                    '午餐': {
                        '菜单': result['每日食谱'].get('lunch', {}).get('菜单', []),
                        '营养': result['每日食谱'].get('lunch', {}).get('营养分析', {})
                    },
                    '晚餐': {
                        '菜单': result['每日食谱'].get('dinner', {}).get('菜单', []),
                        '营养': result['每日食谱'].get('dinner', {}).get('营养分析', {})
                    }
                },
                '运动建议': result.get('运动建议', []),
                '健康贴士': result.get('健康提示', [])
            }
